...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 12 16:35:41 2020
...
def DrugComboDataCombiner():
    for drug_number in range(len(prediction_index)):
        for drug_number2 in range(drug_number + 1, len(prediction_index)):
            for drug_number3 in range(drug_number2 + 1, len(prediction_index)):
                combined_list = row_combiner(X_predict[drug_number], X_predict[drug_number2])
                combined_list = row_combiner(combined_list, X_predict[drug_number3])
                combined_list = [combined_list]
                y_pred = classifier.predict(np.array(combined_list))
                results.append([str(prediction_index[drug_number]), str(prediction_index[drug_number2]), str(prediction_index[drug_number3]), y_pred[0][0]])
        print(drug_number)
...
results_df.to_csv("Three_Drug_Combo_Predictions_Huh7_Fzd2.csv")
